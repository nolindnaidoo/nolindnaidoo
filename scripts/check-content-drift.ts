...
/**
 * Marketplace downloads across the suite, read from the same query that counts
 * the extensions. `downloadCount` is the comparable figure — the `install`
 * statistic counts one channel and reports roughly a tenth of it.
 */
export async function marketplaceDownloads(): Promise<Fetched<number>> {
	type Stat = { statisticName?: string; value?: number };
	type Query = { results?: { extensions?: { statistics?: Stat[] }[] }[] };
	const result = await fetchJson<Query>(
		'https://marketplace.visualstudio.com/_apis/public/gallery/extensionquery',
		{
			method: 'POST',
	...
	if (result.error) return { error: result.error };
	const extensions = result.value.results?.[0]?.extensions ?? [];
	return {
		value: extensions.reduce(
			(sum, extension) =>
				sum +
				Number(
					extension.statistics?.find((stat) => stat.statisticName === 'downloadCount')?.value ?? 0,
				),
			0,
		),
	};
...
export function claimedInstalls(): number {
	...
	return numberIn(row.value, 'download count') * 1000;
}
...
export async function main(): Promise<number> {
	const [extensions, downloads, marketplace, repos] = await Promise.all([
		publishedExtensions(),
		openVsxDownloads(),
	...
	if (marketplace.error) skipped.push(`marketplace downloads — ${marketplace.error}`);
	// The claim covers both registries, so the check has to as well — comparing a
	// combined figure against one channel reports drift that isn't there.
	if (downloads.value !== undefined && marketplace.value !== undefined) {
		checks.push(compare('suite downloads', claimedInstalls(), downloads.value + marketplace.value));
	}
