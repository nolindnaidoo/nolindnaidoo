...
function subjectsInRange(base, head) {
	// An all-zero SHA is what GitHub sends for the first push to a branch:
	// there is no previous commit, so there is no range to check.
	if (!base || /^0+$/.test(base)) return { subjects: [] };

	try {
		const log = execFileSync('git', ['log', '--format=%s', `${base}..${head}`], {
			encoding: 'utf8',
		...
		return { error: `could not read commits in ${base}..${head}: ${detail}` };
	}
}
