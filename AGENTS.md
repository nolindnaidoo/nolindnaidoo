...
**It exists to advance a career to the next level: recruitment by the
highest-level people.** Not consulting leads, not audience building, not
marketing for any other project. Authority and an eventual social launch are
the mechanism; the hire is the point. The reader who matters is someone
deciding on a senior hire and prepared to pay multiple six figures.

That reader reads long, and they are evaluating *judgment* — tradeoffs,
non-goals, what broke and what happened next — rather than output volume. The
rarest thing they cannot learn from an interview loop or a reference check is
how someone acts on finding out they were wrong, so artifacts that show that
are worth disproportionately more than artifacts that show success.

**The open source is the argument.** The enterprise record — industry firsts,
Fortune top-10 engagements, principal and chief roles — is real and permanently
closed. Nobody outside can inspect it. The open source is where the same
engineer becomes *inspectable*, and **maintenance is the proof**: a one-off is
a demo, a maintained fleet with gates that fail on drift is a practice. The
résumé states what was done; the open source shows how, continuously.

Consequences for the design, which are deliberate and not up for optimization:

- **Bounce rate is intentional.** Most visitors leaving without clicking is the
  filter working. The click is the qualification, so there is no lead capture
  and no CTA — the right reader finds contact on their own.
- **Single page, no navigation, ever.** Case studies are a homepage section
  with one detail layer at `/case-studies/<slug>`.
- **Never optimize the writing for a launch spike or a platform algorithm.**
  That is writing for the wrong reader. Reach matters only insofar as it gets
  the right person to land.
