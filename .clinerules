...
**Read it before writing code.** `README.md` is the GitHub profile, not this
project's readme — never write to it as part of site work.
...
- Copy lives in `src/lib/content/`, never in markup.
...
- Every fact has one home. Before adding a constant, check whether
  `src/lib/content/` already owns it — drift is this codebase's failure mode.
